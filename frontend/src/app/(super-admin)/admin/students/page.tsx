...
export default function EnrolledStudentsManagementPage() {
  ...
  const [metrics, setMetrics] = useState({ totalStudents: 0, totalInstitutes: 0, totalEnrollments: 0 });
  ...
  const fetchStudentsData = async () => {
    try {
      ...
      if (analytics?.groupedStudents) {
        setGroupedStudents(analytics.groupedStudents);
        const initialOpen: Record<string, boolean> = {};
        analytics.groupedStudents.forEach((g: any) => {
  ...
  // Filter grouped students data
  const filteredGroups = groupedStudents.filter((group) => {
    ...
    return matchesInstName || matchesStudent;
  });

  const exportToCSV = () => {
    const headers = ['ID', 'First Name', 'Last Name', 'Email', 'Phone', 'Institute Name', 'Enrolled Courses', 'Status'];
    const rows: any[] = [];
    groupedStudents.forEach((group) => {
      group.students.forEach((s: any) => {
        rows.push([s.id, s.firstName, s.lastName, s.email, s.phone || '', group.instituteName, s.enrollmentsCount, s.isActive ? 'Active' : 'Suspended']);
      });
    });
    ...
    link.setAttribute('download', `enrolled_students_roster_${Date.now()}.csv`);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
              ...
              <span className="text-emerald-400 font-semibold">Enrolled Students</span>
            </div>
            <h1 className="text-lg font-bold text-white tracking-tight">Enrolled Students Roster</h1>
          </div>
        </div>
      ...
      <main className="p-6 max-w-7xl w-full mx-auto space-y-6 flex-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-lg">
            ...
            <p className="text-xs text-slate-400 mt-2">Active student accounts across platform</p>
          </div>
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-lg">
            ...
            <p className="text-xs text-slate-400 mt-2">Institutes with enrolled students</p>
          </div>

          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-lg">
            <div className="flex items-center justify-between text-teal-400">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Active Enrollments</span>
              <div className="p-2 rounded-xl bg-teal-500/10 border border-teal-500/20 text-teal-400">
                <CheckCircle className="w-5 h-5" />
              </div>
            </div>
            <div className="text-3xl font-black text-white mt-3">{metrics.totalEnrollments}</div>
            <p className="text-xs text-slate-400 mt-2">Active course enrollments</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              placeholder="Search student name, email, or institute..."
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="p-12 text-center bg-slate-900/80 border border-slate-800 rounded-2xl space-y-2">
              ...
              <p className="text-sm font-semibold text-slate-300">No Students Match Your Search</p>
              <p className="text-xs text-slate-500">Try adjusting your search query or institute dropdown filter.</p>
            </div>
          ) : (
            filteredGroups.map((group) => {
              const studentsList = group.students.filter(
                (s: any) =>
                  `${s.firstName} ${s.lastName}`.toLowerCase().includes(search.toLowerCase()) ||
                  s.email.toLowerCase().includes(search.toLowerCase()) ||
                      ...
                      <div className="text-left">
                        <h3 className="text-sm font-bold text-white flex items-center gap-2">
                          ...
                          <span className="px-2.5 py-0.5 rounded-full bg-slate-800 text-emerald-300 font-mono text-[11px] border border-slate-700">
                            {studentsList.length} Student(s)
                          </span>
                        </h3>
                        <p className="text-[11px] text-slate-400 mt-0.5">Enrolled student rosters under this workspace</p>
                      </div>
                    </div>

                    <div className="flex items-center gap-2 text-slate-400">
                      <span className="text-xs font-medium">{isOpen ? 'Hide Details' : 'View Student Roster'}</span>
                  ...
                  {isOpen && (
                    <div className="border-t border-slate-800/80 bg-slate-950/60 p-4">
                      {studentsList.length === 0 ? (
                        <p className="text-xs text-slate-500 text-center py-4">No students matched inside {group.instituteName}.</p>
                      ) : (
                              ...
                              {studentsList.map((student: any) => (
                                <tr key={student.id} className="hover:bg-slate-800/50 transition group">
                                  ...
                                  <td className="py-3 px-4 font-bold text-emerald-400">
                                    {student.enrollmentsCount} Course(s)
                                  </td>
